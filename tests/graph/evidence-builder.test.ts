...
const decision = (graph: ReturnType<typeof buildGraphFromEvidence>) => evaluatePolicySync({ delta: computeGraphDelta(buildBaselineGraph(), graph), proposedGraph: graph, approvedPartners: partners, policyVersion: "test" }).decision;
...
describe("ActivityGraphBuilder evidence path", () => {
  it("maps real extracted Partner X evidence to the frozen review obligation", () => {
    ...
    expect(decision(graph)).toBe("REVIEW");
  });
  ...
  it("maps real extracted transfer evidence to the frozen pool pass-through topology", () => {
    ...
    expect(decision(graph)).toBe("BLOCK");
  });
